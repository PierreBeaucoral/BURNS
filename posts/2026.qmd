...
subtitle: "EFFIS rapid perimeters, as of 10 August 2026"
...
feel that way? The 2026 fire season is about ten weeks old, and this page is
...
where the data stops, on `r as_of_lab`. Ten weeks in, 2026 is
`r if (is.na(pct_vs_med)) "not yet comparable to the median (too early in the season for a meaningful ratio)" else sprintf("running above the median (about %.1f times it)", pct_vs_med / 100)`
and, until the day before this snapshot, above the band entirely. For the
thirteen days from 28 July to 9 August the orange line ran clear of the grey:
not near the top of the range of the last nine seasons but outside it, meaning
no season since 2017 had burned this much by that point in the calendar. That
is the honest headline of this fortnight, and it is worth stating before the
ranking that follows.

The ranking changed on the last day. On 10 August the 2025 trajectory added
some 177,000 hectares in one day and overtook 2026, which is what leaves this
season reading as *the second-worst start in a decade, behind only 2025*. That
order is one day old. It is also thin: 2022 sits about two per cent behind
2026, close enough to reclaim second on a single bad afternoon. Most of a
European fire season happens in July and August. This line has most of its
story still ahead of it, and I will not call a season in week ten.

### The same season, played back

The chart above compresses ten weeks into one frame, which is exactly what
...
over Ávila: 44,403 hectares in Spain and 39,044 in France on the same day. The
line jumps 92,000 hectares in one step, and the readout above it goes to 597%
of a typical season. That single day is most of the gap between 2026 and an
ordinary year.

::: {#fig-race}
![](/assets/anim/2026-race.gif){fig-alt="Two-panel animation: a map of Europe accumulating wildfire perimeters day by day beside the cumulative burned-area chart drawing itself against the 2017-2025 range."}
...
On the map, Spain's fires overwhelmingly break new ground, while France and
Portugal fall more often on ground that has already burned since 2017.
...
navaluenga <- fs_df |> dplyr::filter(grepl("Navaluenga", dplyr::coalesce(commune, ""))) |>
  dplyr::slice_max(area_ha, n = 1, with_ties = FALSE)
sanmartin <- fs_df |> dplyr::filter(grepl("Valdeiglesias", dplyr::coalesce(commune, ""))) |>
  dplyr::slice_max(area_ha, n = 1, with_ties = FALSE)
...
line has pulled clear of every recent summer, 2022 included. A single fire is most
...
Much of the late-July surge sits in the mountains ringing Madrid: the Sierra de
Gredos and the ranges north of the capital. The single largest fire of the
entire 2026 season so far, about
**`r comma(round(navaluenga$area_ha, -3))` hectares** at Navaluenga in Ávila,
burned there on `r date_lab(navaluenga$ba_date)`, and San Martín de
Valdeiglesias, inside the Comunidad de Madrid itself, added about
`r comma(round(sanmartin$area_ha, -3))` hectares the next day. Taken together,

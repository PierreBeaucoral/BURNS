...
The 2026 fire season is about four weeks old. This page is a snapshot of it:
what has burned so far, where, and how that compares with recent seasons at
the same point in the calendar. It is a **state of the season**, not a
verdict.
...
`r round(season_ha / 10540)` times the area of the city of Paris, and about
**`r sprintf("%.0f%%", pct_vs_med)`** of what a typical recent season had
burned by this date (`r comma(round(median_ha, -3))` hectares). But 2026 did
not start in June. Counting from **1 January**, the total is already about
**`r comma(round(full_ha, -3))` hectares**. An unusually busy late winter and
spring did most of that. The chart below sticks to the summer window, so that
seasons are compared like for like; the maps further down show everything
mapped in 2026 so far.
...
where the data stops, on `r as_of_lab`. One month in, 2026 is running above
the median (about `r sprintf("%.1f", pct_vs_med / 100)` times it) but well
...
`r lab_si_ha(peak_2026$area_ha)` burned in that ISO week alone. The grey band
...
p_base +
  ...
  labs(caption = "EFFIS rapid perimeters (≈ ≥30–50 ha), countries tagged by max overlap.") +
...
p_base +
  ...
  labs(caption = "EFFIS rapid perimeters (≈ ≥30–50 ha). Areas from geometry in EPSG:3035.") +
  theme_burns(base_size = 11, map = TRUE) +
  theme(strip.text = element_text(face = "bold"))
```

Two things jump out. First, the familiar geography is already in place: the
Iberian Peninsula and the Mediterranean arc dominate the map. Second, and
this surprised me, most of 2026's burned area so far predates the summer.
About `r comma(round(full_ha - season_ha, -3))` of the
`r comma(round(full_ha, -3))` hectares were mapped between January and May,
an unusually active late winter and spring, visible in the left panel above.
Keep that in mind when reading the summer chart at the top: it deliberately
...
fast; the tiniest border slivers, about 1% of the total area, drop out in the
process. For survey-grade outlines, use the official EFFIS viewer.

```{r}
#| label: leaflet

leaf_sf <- cached(sprintf("leaflet_2026_asof%s", format(as_of, "%Y%m%d")), {
  x <- tagged_2026_full |>
...
Numbers like "seven thousand hectares" are hard to picture. Below are the
ten largest 2026 fires so far, each drawn at the exact same scale, so size
differences are visible rather than just stated. A single grey circle, equal
in area to the city of Paris, sits alongside them for reference.
...
p_raw + p_pct +
  ...
  plot_annotation(
    caption = "EFFIS rapid perimeters (≈ ≥30–50 ha), Jan 1 – season to date. Point/line colors from national flags.",
    theme = theme(plot.caption = element_text(colour = "grey30"))
  )
```
...
ggplot() +
  ...
  labs(caption = "EFFIS rapid perimeters; historical footprint = union of all 2017–2025 perimeters, Europe-clipped.") +
  theme_burns(base_size = 11, map = TRUE) +
...
burned at least once since 2017. Fire coming back to recent scars is common
in Mediterranean landscapes: vegetation regrows fast, and what regrows burns
easily. Still, treat the exact figure as an estimate. Both layers are rapid
mapping products, and I simplified the shapes to keep the computation
tractable. On the 2025 post's origin story:
`r if (aude_n > 0) sprintf("the Aude département in France does show 2026 activity: %d mapped fire%s totalling about %s hectares so far.", aude_n, if (aude_n > 1) "s" else "", comma(round(aude_ha, -2))) else "the Aude département in France shows no mapped 2026 fire in this snapshot so far (an honest zero, not a data gap)."`
...
Is this season eating into different vegetation than usual? To compare
fairly, I use identical calendar windows: what burned between **1 June and
